...
class Composer
{
    ...
    public static function symLinkTpl(Event $event)
    {
        ...
        $target = $installPath . '/../../tpl';

        /* symlink the tpl files from the teleport library */
        $sources = new Finder();
        $sources->files()->ignoreVCS(true)->ignoreDotFiles(true)->name('*.tpl.json')->name('*.php')->in(__DIR__ . '/../../tpl');

        foreach ($sources as $source) {
            self::createSymLink($event, __DIR__ . '/../../tpl', $source, $target);
        }
    }
    ...
    protected static function createSymLink($event, $base, $source, $target)
    {
        ...
        $event->getIO()->write("symlinking {$filename} -> {$target}");
